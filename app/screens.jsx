...
// Format uptime seconds → "3d 14h 22m" / "5h 12m" / "47m"
...
// Format bytes → "412 MB" / "1.2 GB" / "412 / 1024 MB"
function fmtBytes(b) {
  b = Number(b) || 0;
...
function fmtMem(usedB, totalB) {
  if (!totalB) return "--";
  return Math.round(usedB / (1024 * 1024)) + " / " +
         Math.round(totalB / (1024 * 1024)) + " MB";
}
// 142000 → "142K", 1240000 → "1.2M"
    ...
    <div className="fade-in">
      <div className="row between" style={{ marginBottom: 18 }}>
        <div>
          ...
          <div className="dim">{t("dashboardSub")} · <span className="mono">{sys.model || "openwrt"}</span></div>
        </div>
        <div className="row" style={{ gap: 8 }}>
          <Pill tone="ok">{t("online")}</Pill>
          <span className="mono tiny dim">↑ {fmtUptime(sys.uptime)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-2" style={{ marginBottom: 16 }}>
        ...
        <div className="card" onClick={() => go("modem")} style={{ cursor: "pointer" }}>
          <div className="card-head">
            ...
            <div style={{ marginLeft: "auto" }}><Pill tone="info">{mob.mode || "--"}</Pill></div>
          </div>
          <div className="row" style={{ alignItems: "flex-end", gap: 18, marginBottom: 16 }}>
            <div>
              <div className="metric-label">{t("operator")}</div>
              <div style={{ fontSize: 22, fontWeight: 700, fontFamily: "var(--font-display)" }}>{mob.operator || "--"}</div>
              <div className="dim small">{mob.model || "--"}</div>
            </div>
          ...
          <div className="grid grid-4" style={{ gap: 8 }}>
            <Mini label="Band" value={mob.band || "--"} />
            <Mini label="IP" value={mob.ip || "--"} />
        ...
        <div className="card">
          <div className="card-head">
            ...
            <div style={{ marginLeft: "auto" }} className="mono tiny dim">{sys.model || "OpenWrt"}</div>
          </div>
          ...
          <div className="grid grid-3" style={{ gap: 8 }}>
            ...
            <Mini label="Mem" value={fmtMem(sys.ramUsed, sys.ramTotal)} />
            <Mini label="↑" value={fmtUptime(sys.uptime)} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-2">
        <div className="card" onClick={() => go("multiwan")} style={{ cursor: "pointer" }}>
          <div className="card-head">
            ...
            <div style={{ marginLeft: "auto" }}>
              {(() => {
                const ws = window.VData.state.wans || [];
                const up = ws.filter(w => w.status === "online").length;
                return <Pill tone={up === ws.length && up > 0 ? "ok" : up > 0 ? "warn" : "bad"}>{up}/{ws.length || 0}</Pill>;
              })()}
            </div>
          </div>
        ...
        <div className="card" onClick={() => go("adguard")} style={{ cursor: "pointer" }}>
          <div className="card-head">
            ...
            <div style={{ marginLeft: "auto" }}>
              <Pill tone={d.agh && d.agh.enabled ? "ok" : "bad"}>
                {d.agh && d.agh.enabled ? t("connected") : t("offline")}
              </Pill>
            </div>
          </div>
          <div className="grid grid-4" style={{ gap: 8, marginBottom: 14 }}>
            <Mini label={t("queries")} value={fmtCompact((d.agh && d.agh.queries) || 0)} color="var(--info)" />
            <Mini label={t("blocked")} value={fmtCompact((d.agh && d.agh.blocked) || 0)} color="var(--danger)" />
            <Mini label={t("blockedPct")} value={((d.agh && d.agh.blockedPct) || 0).toFixed(1) + "%"} color="var(--warn)" />
            <Mini label={t("avgTime")} value={Math.round((d.agh && d.agh.avgTime) || 0) + "ms"} color="var(--accent)" />
          </div>
